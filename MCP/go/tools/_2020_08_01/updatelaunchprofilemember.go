...
func UpdatelaunchprofilememberHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/2020-08-01/studios/%s/launch-profiles/%s/membership/%s", cfg.BaseURL, launchProfileId, principalId, studioId)
		...
		var result models.UpdateLaunchProfileMemberResponse
...
func CreateUpdatelaunchprofilememberTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("patch_2020-08-01_studios_studioId_launch-profiles_launchProfileId_membership_principalId",
		mcp.WithDescription("Update a user persona in launch profile membership."),
		...
		mcp.WithString("persona", mcp.Required(), mcp.Description("Input parameter: The persona.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    UpdatelaunchprofilememberHandler(cfg),
	}
}
