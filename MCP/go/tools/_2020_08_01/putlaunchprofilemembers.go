...
func PutlaunchprofilemembersHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/2020-08-01/studios/%s/launch-profiles/%s/membership", cfg.BaseURL, launchProfileId, studioId)
		...
		var result models.PutLaunchProfileMembersResponse
...
func CreatePutlaunchprofilemembersTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_2020-08-01_studios_studioId_launch-profiles_launchProfileId_membership",
		mcp.WithDescription("Add/update users with given persona to launch profile membership."),
		...
		mcp.WithString("identityStoreId", mcp.Required(), mcp.Description("Input parameter: The ID of the identity store.")),
		mcp.WithArray("members", mcp.Required(), mcp.Description("Input parameter: A list of members.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    PutlaunchprofilemembersHandler(cfg),
	}
}
