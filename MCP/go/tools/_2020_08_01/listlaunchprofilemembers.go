...
func ListlaunchprofilemembersHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/2020-08-01/studios/%s/launch-profiles/%s/membership%s", cfg.BaseURL, launchProfileId, studioId, queryString)
		...
		var result models.ListLaunchProfileMembersResponse
...
func CreateListlaunchprofilemembersTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_2020-08-01_studios_studioId_launch-profiles_launchProfileId_membership",
		mcp.WithDescription("Get all users in a given launch profile membership."),
	...
	return models.Tool{
		Definition: tool,
		Handler:    ListlaunchprofilemembersHandler(cfg),
	}
}
