...
func UntagresourceHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/2020-08-01/tags/%s#tagKeys%s", cfg.BaseURL, resourceArn, queryString)
...
func CreateUntagresourceTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("delete_2020-08-01_tags_resourceArn#tagKeys",
		mcp.WithDescription("Deletes the tags for a resource."),
		mcp.WithString("resourceArn", mcp.Required(), mcp.Description("Identifies the Amazon Resource Name(ARN) key from which you are removing tags. ")),
		mcp.WithArray("tagKeys", mcp.Required(), mcp.Description("One or more tag keys. Specify only the tag keys, not the tag values.")),
	)
