...
func DeletestreamingsessionHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/2020-08-01/studios/%s/streaming-sessions/%s", cfg.BaseURL, sessionId, studioId)
		...
		var result models.DeleteStreamingSessionResponse
...
func CreateDeletestreamingsessionTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("delete_2020-08-01_studios_studioId_streaming-sessions_sessionId",
		mcp.WithDescription("<p>Deletes streaming session resource.</p> <p>After invoking this operation, use GetStreamingSession to poll the resource until it transitions to a <code>DELETED</code> state.</p> <p>A streaming session will count against your streaming session quota until it is marked <code>DELETED</code>.</p>"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    DeletestreamingsessionHandler(cfg),
	}
}
