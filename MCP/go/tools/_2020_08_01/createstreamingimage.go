...
func CreatestreamingimageHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/2020-08-01/studios/%s/streaming-images", cfg.BaseURL, studioId)
		...
		var result models.CreateStreamingImageResponse
...
func CreateCreatestreamingimageTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_2020-08-01_studios_studioId_streaming-images",
		mcp.WithDescription("Creates a streaming image resource in a studio."),
		...
		mcp.WithString("ec2ImageId", mcp.Required(), mcp.Description("Input parameter: The ID of an EC2 machine image with which to create this streaming image.")),
		mcp.WithString("name", mcp.Required(), mcp.Description("Input parameter: A friendly name for a streaming image resource.")),
	...
	return models.Tool{
		Definition: tool,
		Handler:    CreatestreamingimageHandler(cfg),
	}
}
