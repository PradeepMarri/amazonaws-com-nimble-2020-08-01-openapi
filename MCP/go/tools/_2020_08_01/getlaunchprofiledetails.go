...
func GetlaunchprofiledetailsHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/2020-08-01/studios/%s/launch-profiles/%s/details", cfg.BaseURL, launchProfileId, studioId)
		...
		var result models.GetLaunchProfileDetailsResponse
...
func CreateGetlaunchprofiledetailsTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_2020-08-01_studios_studioId_launch-profiles_launchProfileId_details",
		mcp.WithDescription("Launch profile details include the launch profile resource and summary information of resources that are used by, or available to, the launch profile. This includes the name and description of all studio components used by the launch profiles, and the name and description of streaming images that can be used with this launch profile."),
	...
	return models.Tool{
		Definition: tool,
		Handler:    GetlaunchprofiledetailsHandler(cfg),
	}
}
