...
func PutstudiomembersHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/2020-08-01/studios/%s/membership", cfg.BaseURL, studioId)
		...
		var result models.PutStudioMembersResponse
...
func CreatePutstudiomembersTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_2020-08-01_studios_studioId_membership",
		mcp.WithDescription("Add/update users with given persona to studio membership."),
	...
	return models.Tool{
		Definition: tool,
		Handler:    PutstudiomembersHandler(cfg),
	}
}
