...
func CreatestreamingsessionstreamHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/2020-08-01/studios/%s/streaming-sessions/%s/streams", cfg.BaseURL, sessionId, studioId)
		...
		var result models.CreateStreamingSessionStreamResponse
...
func CreateCreatestreamingsessionstreamTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_2020-08-01_studios_studioId_streaming-sessions_sessionId_streams",
		mcp.WithDescription("<p>Creates a streaming session stream for a streaming session.</p> <p>After invoking this API, invoke GetStreamingSessionStream with the returned streamId to poll the resource until it is in the <code>READY</code> state.</p>"),
		...
		mcp.WithNumber("expirationInSeconds", mcp.Description("Input parameter: The expiration time in seconds.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    CreatestreamingsessionstreamHandler(cfg),
	}
}
